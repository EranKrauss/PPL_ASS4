...
const checkEqualType = (
  ...
  te1 === undefined
    ? bind(T.unparseTExp(te2), (texp: string) =>
        makeFailure(`Incompatible types: undefined - ${texp}`)
      )
    ...
    : // L51
    T.isClassTExp(te1) && T.isClassTExp(te2)
    ? checkClassEqualTypes(te1, te2, exp)
    : // A class can match a proc (symbol -> T) if the symbol is bound and is in the interface of the class
    T.isClassTExp(te1) && T.isProcTExp(te2)
    ? checkClassProcEqualTypes(te1, te2, exp)
    : T.isClassTExp(te2) && T.isProcTExp(te1)
    ? checkClassProcEqualTypes(te2, te1, exp)
    : safe2((te1: string, te2: string) =>
...
// L51
const checkClassEqualTypes = (
  te1: T.ClassTExp,
  te2: T.ClassTExp,
  exp: A.Exp
): Result<true> => {
  if (te1.methods.length != te2.methods.length) {
    return safe2((te1: string, te2: string) =>
      makeFailure<true>(`Wrong number of methods ${te1} - ${te2}`)
    )(T.unparseTExp(te1), T.unparseTExp(te2));
  } else {
    const checks = mapResult(
      (method) =>
        safe2((me1: T.TExp, me2: T.TExp) => checkEqualType(me1, me2, exp))(
          T.classTExpMethodTExp(te1, method),
          T.classTExpMethodTExp(te2, method)
        ),
...
const checkClassProcEqualTypes = (
  ct: T.ClassTExp,
  pt: T.ProcTExp,
  exp: A.Exp
): Result<true> => {
  if (
    pt.paramTEs.length != 1 ||
    !T.isSymbolTExp(pt.paramTEs[0]) ||
    !pt.paramTEs[0].val
  )
    return makeFailure<true>(
      "A class can only match a proc of 1 ground symbol param"
    );
  const method = pt.paramTEs[0].val.val;
  const constraint = bind(T.classTExpMethodTExp(ct, method), (methodTExp) =>
    checkEqualType(methodTExp, pt.returnTE, exp)
  );
...
const checkTVarEqualTypes = (
  ...
  T.tvarIsNonEmpty(tvar)
    ...
    : bind(checkNoOccurrence(tvar, te, exp), (_) => {
    ...
    T.isAtomicTExp(te1)
      ...
      ? T.eqTVar(te1, tvar)
        ? bind(A.unparse(exp), (exp: string) =>
            makeFailure(
              `Occur check error - ${te1.var} - ${tvar.var} in ${exp}`
            )
          )
        : makeOk(true)
      : bind(A.unparse(exp), (exp: string) =>
          makeFailure(`Bad type expression - ${JSON.stringify(te1)} in ${exp}`)
        );
...
export const inferTypeOf = (concreteExp: string): Result<string> =>
  bind(A.parse(concreteExp), (parsed) => {
    const tenv = makeTEnvFromClasses(parsed); // L51
    // console.log(`tenv = ${tenv}`);
    return bind(typeofExp(parsed, tenv), T.unparseTExp);
  });
...
const typeofIf = (ifExp: A.IfExp, tenv: E.TEnv): Result<T.TExp> => {
  ...
  const constraint2 = safe2((thenTE: T.TExp, altTE: T.TExp) =>
...
export const typeofProc = (proc: A.ProcExp, tenv: E.TEnv): Result<T.TExp> => {
  ...
  return bind(constraint1, (_) =>
    makeOk(T.makeProcTExp(argsTEs, proc.returnTE))
  );
};
...
export const typeofApp = (app: A.AppExp, tenv: E.TEnv): Result<T.TExp> => {
  ...
  const constraint = safe2((ratorTE: T.TExp, randsTE: T.TExp[]) =>
...
export const typeofDefine = (
  ...
  tenv: E.TEnv
    ...
    const c : Result<boolean> = bind( typeofExp(exp.val, E.makeExtendTEnv([exp.var.var], [exp.var.texp], tenv)), (valType: T.TExp) => checkEqualType(exp.var.texp, valType, exp))
    return bind(c, (eqTypes: boolean) => makeOk(T.makeVoidTExp()))
};
...
bind(typeofExp(exp, tenv), (currType: T.TExp) : Result<T.TExp> => 
    A.isDefineExp(exp) ? 
        isEmpty(exps) ? makeOk(currType) : typeofProgramExps(first(exps), rest(exps), E.makeExtendTEnv([exp.var.var], [currType], tenv)) 
    : isEmpty(exps) ? makeOk(currType) : typeofProgramExps(first(exps), rest(exps), tenv)
)
...
export const typeofSet = (exp: A.SetExp, tenv: E.TEnv): Result<T.VoidTExp> => {
  const typeVar: Result<T.TExp> = typeofExp(exp.var, tenv)
  const typeVal: Result<T.TExp> = typeofExp(exp.val, tenv)
  const f = safe2((firstType: T.TExp , secondType: T.TExp) => checkEqualTypes([firstType], [secondType], exp))
  return isOk(f(typeVar, typeVal)) ? makeOk(T.makeVoidTExp()) : makeFailure("var and val types is not match")
};
